...
def horizontal_row_max(arr):
        ...
        for row in range(20):   #go to the LAST row
                for col in range(17):   #go to the 4th to last column (i.e. do not fall off the grid)
                        ...
                        print("HORIZONTAL 4 POINTS: \nStart: [{0},{1}] \nSecond:[{2},{3}] \nThird: [{4},{5}] \nForth: [{6},{7}]".format(row,col, row, col+1, row, col+2, row, col+3))
                        print("Current result for starting position [{0},{1}] (vertically) is: {2}".format(row, col, curr_res))

                        print("The values are", (grid[row][col]), "*", (grid[row][col+1]), "*", (grid[row][col+2]), "*", (grid[row][col+3]), "=", curr_res)

                        # print("The values are {1} * {2} * {3} * {4}".format( grid[row][col], grid[row][col+1], grid[row][col+2], grid[row][col+3]))

                        if curr_res > max_res:
                                max_res = curr_res              # optionally could store the current index of the maximum value found so far.
                                print("The current maximum result is", max_res, "\n\n")

        return max_res

horizontal_row_max(grid)
